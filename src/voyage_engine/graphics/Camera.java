...
public class Camera {
    private final Vec3 WORLD_UP = new Vec3(0.0f, 1.0f, 0.0f);
    ...
    private float velocity = 5, sensitivity = 15;
	...
	public void update(double delta) {
        ...
        yaw += Input.currentMouseDelta().x * sensitivity;
        pitch += Input.currentMouseDelta().y * sensitivity;
        // ensure that the pitch doesnt roll over past 89 or beneath -89.
        pitch = (pitch > 89.0f) ? 89.0f : pitch;
        pitch = (pitch < -89.0f) ? -89.0f : pitch;
        // recompute the camera vectors (front, right, up) used to build the view matrix.
        computeCameraVectors();
        // set the camera veleocity based on if the left shift key is held down.
        velocity = (Input.isKeyDown(Key.LEFT_SHIFT)) ? (25.0f * (float) delta) : (5.0f * (float) delta);
    ...
    private void computeCameraVectors() {
		front.x = (float) (Math.cos(Math.toRadians(yaw)) * Math.cos(Math.toRadians(pitch))); 
		front.y = (float) Math.sin(Math.toRadians(pitch)); 
		front.z = (float) (Math.sin(Math.toRadians(yaw)) * Math.cos(Math.toRadians(pitch)));
		front.normalise();
		Vec3.cross(front, WORLD_UP, right);
		right.normalise();
    ...
    private void checkCameraMovement() {
        ...
        if(Input.isKeyDown(Key.SPACE)) {
			Vec3.add(position, new Vec3(WORLD_UP.x * velocity, WORLD_UP.y * velocity, WORLD_UP.z * velocity), position);
        }
